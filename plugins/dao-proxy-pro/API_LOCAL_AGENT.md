...
| GET | `/origin/health` | - | 存活别名（→ping 精简版，通用约定）。**v9.9.289 新增**；旧版（含当前 141 运行实例）返回 404 时请改用 `/origin/ping`。 | 🔧新增 |
| GET | `/origin/selftest` | - | **三径自检**：`canon_path`/`route_path`/`unlock_path` 三路径诊断 + 顶层 `ok`。**v9.9.289 实装**；旧版 404 时用 `/origin/ping`+`/origin/ea/probe` 替代。 | 🔧新增 |
...
| 症状 | 真相 / 处理 |
|---|---|
| `GET /origin/health` 返回 404 | 当前 141 运行的 v9.9.288 尚无此口（**v9.9.289 已在源码实装**，需重建 vsix 后生效）。在那之前 origin 健康端点用 **`/origin/ping`**；`/api/health` 是 Bridge 的，不是 origin 的。 |
| `GET /origin/selftest` 404 | 同上：v9.9.288 未实装，**v9.9.289 源码已实装**（三径自检）。在那之前用 `/origin/ping` + `POST /origin/ea/probe` 替代。 |
...
- `/origin/selftest` + `/origin/health`：**已在源码实装**（本 PR，目标 v9.9.289）—— `health`=ping 精简别名，`selftest`=三径（道魂/外接/解锁）自检。**需重建 vsix 并在 141 重装后才在运行实例生效**；当前 141 跑的 v9.9.288 仍 404，过渡期按 §6 用 ping/probe。
