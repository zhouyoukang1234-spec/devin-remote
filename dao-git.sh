#!/bin/bash
# dao-git - transparent remote command execution over a GIT branch (Linux/Devin VM)
...
#
# Usage: dao-git "command"        -> stdout = result (feels like local execution)
...
[ $# -eq 0 ] && { echo "Usage: dao-git \"command\"" >&2; exit 1; }
...
echo "dao: timeout ${TIMEOUT}s - is agent-git.ps1 running on the user PC?" >&2
exit 1
