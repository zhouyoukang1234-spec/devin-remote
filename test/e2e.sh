#!/bin/bash
# Offline end-to-end test for the git transport (dao-exec.sh sender + agent.ps1 receiver).
...
# Sender helper: dao-exec.sh with its OWN cache. Usage: dex <secret> "<cmd>"
dex(){ DAO_SECRET="$1" DAO_CACHE="$WORK/send-cache-$1" bash "$ROOT/dao-exec.sh" "$2" 2>"$WORK/dex.err"; }
