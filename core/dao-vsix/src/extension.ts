...
.mo{position:fixed;inset:0;background:rgba(0,0,0,.55);display:flex;align-items:center;justify-content:center;z-index:300;padding:20px}
.md{background:var(--card);border:1px solid var(--border);border-radius:8px;padding:16px;width:min(560px,94vw);max-height:88vh;overflow-y:auto;box-shadow:0 8px 32px rgba(0,0,0,.5)}
.md h3{font-size:14px;font-weight:600;margin-bottom:10px}
.md input,.md textarea{background:var(--input);color:var(--input-fg);border:1px solid var(--input-border);border-radius:4px;padding:6px 8px;font-size:12px}
...
// 面板内修改剧本(正文直接可见可编辑) — 与 kEdit 同源, 取代原生 InputBox/临时文档(多窗口下弹不出/看不到内容)。
function pEdit(id){var it=(S.data.playbooks||[]).find(function(p){return String(p.id)===String(id)});if(!it){toast('未找到该剧本',false);return}var ro=((it.access||'')==='community');var title=it.title||it.name||'';var body=(it.body!=null?it.body:(it.content||''));var bodyHtml='<label style="font-size:11px;color:var(--muted)">标题 title</label><input id="m1" '+(ro?'disabled':'')+' value="'+esc(title)+'" style="width:100%;margin:4px 0"><label style="font-size:11px;color:var(--muted)">正文 body (Markdown)</label><textarea id="m2" '+(ro?'disabled':'')+' style="width:100%;height:260px;margin:4px 0;font-family:monospace;white-space:pre">'+esc(body)+'</textarea>'+(ro?'<p style="font-size:10px;color:var(--warn);margin:4px 0">此为官方模板剧本(access=community), 无法修改。</p>':'');sm('✏️ 修改剧本 · '+esc(title),bodyHtml,function(){if(ro)return;var n=document.getElementById('m1').value.trim();if(!n){toast('标题不能为空',false);return false}var b=document.getElementById('m2').value;cmd('devinEditPlaybookInline',{id:String(id),title:n,body:b});toast('保存中…',true)})}
function sw(t){
  S.tab=t;
...
function rT(tab,items,err,fallbackProxy){
  ...
  if(tab==='sessions'){
    ...
    var pbCard=function(p){
      ...
      return '<div class="card"><div class="cr"><span class="l" style="font-weight:500;color:var(--fg)">'+esc(title)+'</span><span class="v"><span class="tag playbook">P</span></span></div>'+(status?'<div style="font-size:10px;color:var(--muted);margin-top:4px">'+esc(status)+'</div>':'')+'<div class="br" style="margin-top:4px">'+lkBtn('playbooks',title)+'<button class="btn sm" onclick="pEdit(&#39;'+esc(String(id))+'&#39;)">✏️ 修改</button><button class="btn sm danger" onclick="cmd(&#39;devinDeletePlaybook&#39;,{id:&#39;'+esc(String(id))+'&#39;})">🗑</button></div></div>';
    };
...
async function handleMiddlePanelMessage(msg: any, context: vscode.ExtensionContext) {
    ...
    try {
        switch (msg.command) {
            ...
            case 'devinEditPlaybookInline': {
                if (!ws.devinOrgId) { refreshReply({ type: 'actionResult', command: 'devinEditPlaybookInline', ok: false }); break; }
                ...
                if (!item) { refreshReply({ type: 'actionResult', command: 'devinEditPlaybookInline', ok: false }); break; }
                const newTitle = String(msg.title || item.title || item.name || '').trim();
                const newBody = String(msg.body != null ? msg.body : (item.body || ''));
                if (!newTitle) { refreshReply({ type: 'actionResult', command: 'devinEditPlaybookInline', ok: false }); break; }
                await devinDeletePlaybook(ws.devinOrgId, String(item.id), ws.devinAuth1);
                const rIn = await devinInjectPlaybook(ws.devinOrgId, newTitle, newBody, ws.devinAuth1);
                if (rIn.ok) { vscode.window.showInformationMessage('Playbook 已修改: ' + newTitle); setManualLock(ws.devinOrgId, 'playbooks', newTitle, true); }
                else vscode.window.showErrorMessage('Playbook 修改失败');
                refreshReply({ type: 'actionResult', command: 'devinEditPlaybookInline', ok: rIn.ok });
                break;
            }
