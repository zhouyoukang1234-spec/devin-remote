...
4. **探活驱动·多窗口不打架**（dao-vsix 3.17.16，本源）：
   - 旧法「本机一变就强刷所有账号」在**多窗口**(每个窗口端口/隧道不同)下会互相刷来刷去打架。
   - 改为 **detection-driven**：巡检时先**探活**账号里"已注入的那条 URL"的隧道源点(`scheme://host/`，不打 `/mcp` 以免 SSE 挂起误判)——
     **能连通(2xx~4xx)就什么都不动**（哪怕是另一窗口注的地址，只要活着就不碰）→ 多窗口天然不打架；
     **只有探到死链(5xx/1033/连不上)或缺失才刷**成本机当前活地址。且"本机活地址自身得先探活通过"才有资格替换(不拿死链换死链)。
   - KB 同理：回读已注 KB 的 `公网URL:` 字段，探活 `/api/health`，错了才重写。
   - 软节流 12s + 25s 巡检 + fs.watch 即时触发。
   - **活体实测验证**(`heal_test.py` 对真实账号 org-733123013…)：
     - PHASE A：账号实际漂回了死链 `wet-groups`(隧道又轮换了) → 探活 DEAD → 自愈到 `plains-gear` ✓
     - PHASE B：人为注入假死链 → 探活 DEAD → 自愈到活地址 ✓
     - PHASE C：再跑 → 探活 ALIVE → 不动(幂等·多窗口安全) ✓
