...
| 2 | **dao-vsix** | Devin 全功能面板（会话/知识库/Playbook/Secret）+ 路由官网 + 内嵌 git/穿透板块 + 启动注入帛书规则 | `dao-vsix/` | 1.3.3 | 主体完成；多窗口隔离 + webview 对等 + SSE 流式直通已实测 |
| 3 | **dao-bridge** | 工作区专属内网穿透（随 IDE 启停 + 实时 MD + Cloudflare 隧道） | `dao-bridge-ext/` | 3.2.0 | live E2E 通过；随 IDE 启停 + 命名隧道（稳定 URL）配置支持 |
| 4 | **devin-git-auth** | 多个 Devin 账号绑定到同一 GitHub 仓库协同 | `devin-git-auth/` | 2.3.2 | 绑定目标态达成；VSIX 已可构建安装；活跃协同待打通 |
| 5 | **rt-flow** | Devin Cloud 接入：批量/自动对话备份 + 全量数据快照 + 一键回归本源(wipe) | `rt-flow/` | 4.5.0 | 12/12 批量备份 + 一键 wipe 全链路真号验证 |
